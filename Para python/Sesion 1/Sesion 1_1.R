...
packages<-c("REAT","readxl", "xlsx","ggplot2", "psych","GGally","dplyr","tidyverse","groupdata2","sf","spData" ,"spdep","mapview","colorRamps","fs","foreign")
...
verifica<- rowSums(participest_wider[ , c(36:67)])
 ...
 verif_part_sect<-particip_sect%>% 
   group_by(Año, CVE_ENT)%>%
   summarise(verifica=sum(share_sect))
